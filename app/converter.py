...
def _validate_and_transform_row(row_data, mapping, decimal_separator, valida_dv):
    ...
    try:
        ...
        if not is_valid: row_errors.append(f"Modelo: {err}")
        transformed_row['modelo'] = transformers.clean_numeric_string(val, 3)

        # --- Número Documento ---
        val = raw_data.get('numero_documento')
        is_valid, err = validators.validate_numeric(val, is_required=True, max_len=50) # Limite atualizado
        if not is_valid: row_errors.append(f"Número Documento: {err}")
        ...
        if not is_valid: row_errors.append(f"CPF/CNPJ Prestador: {err}")
        transformed_row['cpf_cnpj_prestador'] = transformers.clean_numeric_string(val, 14)
        
        # --- CEP Prestador ---
        val = raw_data.get('cep_prestador')
        is_valid, err = validators.validate_cep(val)
        if not is_valid: row_errors.append(f"CEP Prestador: {err}")
        ...
        if not is_valid: row_errors.append(f"DDD: {err}")
        transformed_row['ddd'] = transformers.clean_numeric_string(val, 2)

        # --- Razão Social Prestador ---
        val = raw_data.get('razao_social_prestador')
        transformed_row['razao_social_prestador'] = transformers.clean_alphanumeric(val, 150)
        
        # --- Inscrição Municipal Prestador ---
        val = raw_data.get('inscricao_municipal_prestador')
        transformed_row['inscricao_municipal_prestador'] = transformers.clean_alphanumeric(val, 15)

        # --- Endereço Prestador ---
        val = raw_data.get('endereco_prestador')
        transformed_row['endereco_prestador'] = transformers.clean_alphanumeric(val, 200)

        # --- Número Endereço ---
        val = raw_data.get('numero_endereco')
        is_valid, err = validators.validate_numeric(val, is_required=False, max_len=6)
        if not is_valid: row_errors.append(f"Número Endereço: {err}")
        ...
        if not is_valid: row_errors.append(f"Estado (UF): {err}")
        transformed_row['uf_prestador'] = transformers.clean_alphanumeric(val, 2).upper()

        # --- Data Emissão ---
        val = raw_data.get('data_emissao')
        is_valid, err = validators.validate_date_format(val, is_required=True)
        if not is_valid: row_errors.append(f"Data Emissão: {err}")
        ...
        if not is_valid: row_errors.append(f"Data Pagamento: {err}")
        ...
        if not is_valid: row_errors.append(f"Imposto Retido: {err}")
        transformed_row['imposto_retido'] = transformers.transform_boolean(val)

        # --- Tributado Município ---
        val = raw_data.get('tributado_municipio')
        is_valid, err = validators.validate_boolean_string(val)
        if not is_valid: row_errors.append(f"Tributado Município: {err}")
        ...
        is_valid, err = validators.validate_decimal(val_trib, is_required=True, max_len=10, decimal_separator=decimal_separator)
        if not is_valid: row_errors.append(f"Valor Tributável: {err}")
        transformed_row['valor_tributavel'] = transformers.transform_monetary(val_trib, decimal_separator)

        # --- Valor Documento ---
        val_doc = raw_data.get('valor_documento')
        # Passa o 'decimal_separator' para o validador
        is_valid, err = validators.validate_decimal(val_doc, is_required=True, max_len=10, decimal_separator=decimal_separator)
        if not is_valid: row_errors.append(f"Valor Documento: {err}")
        transformed_row['valor_documento'] = transformers.transform_monetary(val_doc, decimal_separator)

        # --- Alíquota ---
        val = raw_data.get('aliquota')
        # Passa o 'decimal_separator' para o validador
        is_valid, err = validators.validate_aliquota(val, decimal_separator=decimal_separator)
        if not is_valid: row_errors.append(f"Alíquota: {err}")
        transformed_row['aliquota'] = transformers.transform_aliquota(val, decimal_separator)
        
        # --- Item LC ---
        val = raw_data.get('item_lc')
        is_valid, err = validators.validate_item_lc(val)
        if not is_valid: row_errors.append(f"Item LC: {err}")
        ...
        if not is_valid: row_errors.append(f"Unidade Econômica: {err}")
        ...
        is_valid, err = validators.validate_tributavel_vs_documento(
            transformed_row['valor_tributavel'],
            transformed_row['valor_documento']
        )
        if not is_valid: row_errors.append(err)
...
def _generate_header(form_data):
    ...
    try:
        ...
        header_line = ";".join([
            inscricao, mes, ano, campo_4, cod_servico, frase_fixa
        ])
...
def process_conversion(task_id, file_path, form_data, update_status_callback):
    ...
    try:
        ...
        update_status_callback(task_id, 'processing', 25, 'Verificando duplicatas...', '')
        ...
        try:
            ...
            raise Exception("Não foi possível encontrar colunas de Número Documento ou CPF/CNPJ para checar duplicatas.")
        ...
        update_status_callback(task_id, 'processing', 30, 'Iniciando validação...', f'Linha 0 de {total_rows}')
        ...
        for i, (original_index, row) in enumerate(df.iterrows()):
            ...
            if original_index in duplicated_indices:
                row_errors.append("Erro de Duplicidade: Esta nota (Nº Documento + Prestador) está duplicada no arquivo.")
            ...
            if i % 10 == 0 or i == total_rows - 1:
                ...
                update_status_callback(
                    ...
                    'Validando dados...', f'Linha {i + 1} de {total_rows}'
                )
        ...
        filename, write_error = file_handler.generate_txt_file(header_line, final_txt_lines, task_id)
        if write_error:
