...
class ISSNavigator:
    ...
    def select_contribuinte(self, inscricao_municipal: str) -> bool:
        ...
        **Design Pattern: Localização Resiliente**
        ...
        """
        logger.info(
            f"[{self.task_id}] 🏢 Tentando selecionar o Contribuinte: {inscricao_municipal}"
        )
        ...
        try:
            ...
            logger.error(
                f"[{self.task_id}] ❌ Falha na seleção do Contribuinte {inscricao_municipal}: {str(e)}"
            )
            raise NavigationError(
                f"Não foi possível selecionar o Contribuinte {inscricao_municipal} no grid. Detalhes: {e}"
            )
    ...
    def navigate_to_import_page(self) -> None:
        ...
        **Design Pattern: Navegação Direta (Deep Link)**
        ...
        try:
            ...
            raise NavigationError(
                f"Erro ao acessar a URL de Importação: {URLS['importacao']}. Detalhes: {e}"
            )
